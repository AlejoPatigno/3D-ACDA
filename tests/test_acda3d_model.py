...
from acda3d.models import ACDA3D, ACDA3DOutput, build_acda3d
...
def test_complete_forward_contract_and_backward(batch_size):
    model = ACDA3D(3, 8, 6, base_channels=4, concept_hidden_dim=4)
    output = model(torch.randn(batch_size, 1, 16, 16, 16), masks(3))
    assert isinstance(output, ACDA3DOutput)
...
def test_builder_rejects_excluded_architecture_and_validates_roi_count():
    with pytest.raises(ConfigurationError):
        build_acda3d({"name": "3D-ACDA-Full"})
    with pytest.raises(ConfigurationError):
        build_acda3d({"name": "3D-ACDA", "contextual_encoder": True})
    with pytest.raises(ConfigurationError, match="K=3"):
        build_acda3d({"name": "3D-ACDA", "num_rois": 3}, torch.ones(2, 2, 2, 2))
...
def test_phase6_source_dataset_batch_integrates_with_model(tmp_path: Path):
    ...
    batch = dataset[0]
    output = ACDA3D(2, 8, 6, base_channels=4, concept_hidden_dim=4)(
...
def test_production_configuration_builds_102_roi_model():
    model = build_acda3d({
        "model": {
            "name": "3D-ACDA", "contextual_encoder": False, "num_rois": 102,
