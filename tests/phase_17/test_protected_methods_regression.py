...
from acda3d.evaluation.schemas import MethodId
...
from acda3d.experiments.mmd import MMD_DISPLAY_NAME, load_mmd_config, run_mmd_both_directions
...
def test_prototype_pseudo_and_method_scoped_outputs_remain_protected(tmp_path: Path) -> None:
    ...
    assert config.display_name == "3D-ACDA"
...
def test_phase15_and_phase16_boundaries_are_not_reintroduced() -> None:
    ...
    assert "concept_interventions" not in {path.name for path in (root / "src" / "acda3d" / "evaluation").iterdir()}
    assert not (root / "src" / "acda3d" / "phase18").exists()
