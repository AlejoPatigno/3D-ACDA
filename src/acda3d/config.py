"""Typed YAML configuration for 3D-ACDA."""
...
from acda3d.binary import BINARY_CLASS_ORDER, BINARY_CLASS_TO_INDEX, BINARY_MAPPING_CONTRACT
from acda3d.exceptions import ConfigurationError, UnsupportedExperimentError
from acda3d.paths import is_forbidden_hardcoded_path, resolve_path

PROPOSED_MODEL_NAME = "3D-ACDA"
    ...
    def validate(self) -> None:
        ...
        if self.model.contextual_encoder:
            raise ConfigurationError(
                "The contextual ROI encoder belongs to the excluded architecture and cannot be enabled in 3D-ACDA."
            )
        ...
        if method == "baseline" and self.model.name in {"3D-ACDA-Lite", "3D-ACDA-Full", "Lite", "Full"}:
            raise ConfigurationError(f"Invalid public model name: {self.model.name}")

        if self.training.early_stopping:
            raise ConfigurationError(
                "Early stopping is disabled. 3D-ACDA experiments use a fixed number of epochs."
            )
