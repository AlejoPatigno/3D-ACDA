...
from acda3d.evaluation.schemas import AnalysisMode, AuthorizationGateError, canonical_sha256
from acda3d.exceptions import ConfigurationError
from acda3d.models.acda3d import ACDA3D
from acda3d.models.roi_mask_preparation import prepare_feature_grid_roi_masks
...
def _build_checkpoint_bundle(
    ...
    try:
        ...
        raise ConfigurationError(
            f"Checkpoint has incompatible model state for 3D-ACDA: {error}"
        ) from error
    model.to(device)
    model.eval()
...
def run_subject_inference(
    ...
    *,
    ...
    Args:
        model: 3D-ACDA model in eval mode
