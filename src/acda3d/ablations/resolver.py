...
def _parse_model(value: object) -> ModelVariant:
    ...
    if components is None:
        components = ModelVariant(name="3D-ACDA", aggregator="AttentionAggregator").architecture_components
...
def _validate_model(base: AblationBaseConfig, spec: AblationSpec, candidate: str) -> None:
    model = base.model
    if model.name in {"Full", "3D-ACDA-Full", "3D-ACDA-Lite", "Lite", "Contextual"} or model.contextual_encoder is not None or model.runtime_variant_switch:
    ...
    if model.name != "3D-ACDA" or model.aggregator != "AttentionAggregator":
        raise _error("unapproved_override", "base model must be canonical 3D-ACDA with AttentionAggregator", candidate, field="model")
