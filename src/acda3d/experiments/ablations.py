...
from acda3d.binary import binary_ablation_plan
from acda3d.exceptions import ExperimentValidationError
from acda3d.experiments.run_manifest import ablation_output_path, atomic_json, sha256_file
from acda3d.losses import CoreACDA3DLoss
from acda3d.models import build_acda3d, prepare_feature_grid_roi_masks
from acda3d.models.ablations import build_mean_pool_model
...
def load_ablation_config(path: str | Path, *, output_root: str | Path | None = None) -> AblationExperimentConfig:
    ...
    base.setdefault("base_method", "3D-ACDA")
...
def _write_initial_artifacts(run_dir: Path, identity: dict[str, Any], resolved: Any) -> None:
    config_payload = {
        **_artifact_envelope(identity, "resolved_configuration", "phase17.config.v1"),
        "method": {"base_method": "3D-ACDA", "model_variant": resolved.model_variant.to_dict()},
...
def _validate_synthetic(
    ...
    if resolved.candidate_id == "mean_pool":
        ...
        model = build_acda3d({**config.model, "contextual_encoder": False})
    model.eval()
