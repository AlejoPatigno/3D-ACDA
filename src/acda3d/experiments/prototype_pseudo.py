"""Configuration and orchestration for canonical 3D-ACDA prototype/pseudo-label adaptation."""
...
from acda3d.experiments.runner import FoldExecutionResult
...
PROTOTYPE_PSEUDO_DISPLAY_NAME = "3D-ACDA"
...
class PrototypePseudoExperimentRunner(CORALExperimentRunner):
    ...
    def _completed_reuse(self, run_dir: Path, experiment_hash: str, prepared: PreparedCORALFold):
        from acda3d.experiments.runner import SourceOnlyExperimentRunner
    ...
    def run_fold(self, fold: int, seed: int, *, dry_run: bool = False, validate_only: bool = False, resume_from: str | Path | None = None, interrupt_after_epoch: int | None = None):
        ...
        loss_fn = CoreACDA3DLoss(int(self.config.model["num_rois"]), weights=self.config.loss_weights, label_smoothing=self.config.label_smoothing)
