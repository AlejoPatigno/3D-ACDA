...
from acda3d.ablations.schemas import ModelVariant, sha256_payload
...
MEAN_POOL_MODEL_VARIANT = ModelVariant(
    name="3D-ACDA+MeanPoolAggregator",
...
class MeanPoolACDA3D(nn.Module):
    """3D-ACDA with one explicit, non-contextual mean-pool composition."""
    ...
    def __init__(
        ...
        if num_classes not in {2, 3}:
            raise ModelContractError("3D-ACDA supports only binary or historical three-class heads.")
...
def build_mean_pool_model(**kwargs: object) -> MeanPoolACDA3D:
    """Build the explicit approved ``mean_pool`` architecture composition."""
    return MeanPoolACDA3D(**kwargs)
