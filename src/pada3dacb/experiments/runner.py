...
from pada3dacb.data.datasets import LabeledSourceDataset, LabeledTargetDataset
...
from pada3dacb.data.splits import assignment_hash, validate_split_assignments
...
from pada3dacb.experiments.source_only import SourceOnlyExperimentConfig
...
class SourceOnlyExperimentRunner:
    ...
    def _model_and_masks(self, prepared: PreparedFold):
        model = build_pada3dacb(self.config.model)
        ...
        *,
        ...
        try:
            ...
            (run_dir / "log.txt").write_text(
                "PADA-3DACB Source-Only completed. Target metrics are monitoring only.\n",
                encoding="utf-8",
            )
            update_run_manifest(
                manifest_path,
