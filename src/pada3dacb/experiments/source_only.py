"""Typed configuration and fold planning for PADA-3DACB Source-Only."""
...
from pada3dacb.experiments.run_manifest import stable_hash
from pada3dacb.losses import CoreLossWeights
from pada3dacb.training import FixedEpochTrainingConfig
...
class SourceOnlyExperimentConfig:
    name: str = "pada3dacb_source_only"
    ...
    def _validate_common(self) -> None:
        ...
        if self.model.get("name") != "PADA-3DACB" or self.model.get("contextual_encoder") is not False:
            raise ConfigurationError("Source-only requires explicit non-contextual PADA-3DACB.")
...
def load_source_only_config(
    ...
    *,
    ...
    config = SourceOnlyExperimentConfig(
        name=str(experiment.get("name", "pada3dacb_source_only")),
