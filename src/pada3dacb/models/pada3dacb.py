"""Direct production implementation of the former Lite PADA-3DACB path."""
...
PUBLIC_MODEL_NAME = "PADA-3DACB"
CLASS_ORDER = ("CN", "MCI", "AD")


@dataclass(frozen=True)
class PADA3DACBOutput(Mapping[str, torch.Tensor]):
...
class PADA3DACB(nn.Module):
    """Explicit PADA-3DACB model with no contextual ROI encoder."""
...
def build_pada3dacb(config: Any, roi_masks: torch.Tensor | None = None) -> PADA3DACB:
    ...
    if data.get("contextual_encoder", False) is not False:
        raise ConfigurationError("contextual_encoder must be false for PADA-3DACB.")
    if int(data.get("input_channels", 1)) != 1:
        raise ConfigurationError("PADA-3DACB requires one MRI input channel.")
...
@classmethod
def from_config(cls, config: Any) -> PADA3DACB:
    """Create PADA3DACB from configuration dictionary or object."""
    from pada3dacb.config import ModelConfig
    if isinstance(config, ModelConfig):
        config = config.model_dump()
    elif hasattr(config, "model"):
        config = config.model
    return build_pada3dacb(config)


PADA3DACB.from_config = from_config
