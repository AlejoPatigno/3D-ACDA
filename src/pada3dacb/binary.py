...
OASIS_SUBJECT_HASH_KEY_ENV = "PADA3DACB_OASIS_SUBJECT_HASH_KEY_FILE"
OASIS_SEMANTIC_AUTHORITY_MARKER = "PADA3DACB-OASIS-SEMANTIC-REVIEW"
...
def load_verified_oasis_metadata(
    ...
    *,
    ...
    ``PADA3DACB_OASIS_SUBJECT_HASH_KEY_FILE`` (or ``subject_hash_key_file``).
...
def load_binary_checkpoint(model: Any, checkpoint: Any) -> Any:
    ...
    from pada3dacb.models.checkpoint_migration import _locate_state_dict
...
def build_binary_baseline(name: str, config: Mapping[str, Any]) -> Any:
    ...
    from pada3dacb.models.baselines.faster_snn import FasterSNNBaseline
    from pada3dacb.models.baselines.roi_aware_gating import ROIAwareGatingBaseline
...
def build_binary_ablation(
    ...
    *,
    ...
    model_values.setdefault("name", "PADA-3DACB")
    ...
    model = build_mean_pool_model(**kwargs) if candidate_id == "mean_pool" else build_pada3dacb({
        "task_id": "cn_vs_impaired",
        "task_type": "binary_classification",
        "class_order": list(BINARY_CLASS_ORDER),
