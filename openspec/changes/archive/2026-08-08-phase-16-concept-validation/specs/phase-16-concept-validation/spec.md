...
## Planning mirror and ownership constraints

This capability specification is mirrored by the phase proposal, design, tasks, and repository agent plan. The mirror set is maintained by implementation-owned WU-13 and does not expand the scientific scope.

- The authoritative capability mirror path is `openspec/changes/phase-16-concept-validation/specs/phase-16-concept-validation/spec.md`; each planning path has one owner and each work unit declares dependencies, verification, and rollback boundaries.
- The approved delivery path is `auto-chain` with `feature-branch-chain`; each slice is measured against the 400-authored-line ceiling without a size exception.
- Native receipt #1793 is administrative only: it blocks branches, commits, PRs, archive, release, and publication, but not planning maintenance or synthetic implementation/verification.
- The real-run gate remains closed (`authorized: false`), CFS/ACS/PCS/QIS remain `BLOCKED` without authoritative equations, AAGN/FasterSNN remain not applicable, and no training/adaptation or Phase 17 production path is authorized.